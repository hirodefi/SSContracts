...
interface StarSwapFeeHandler {
    function checkFeeLiquidityBalance() external;
    function getOperator() external view returns (address);
}
...
contract StarSwapToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public transferTaxRate = 0;         // percent of transaction amount, 1000 -> 10%
    uint256 public burnRate = 0;                // percent of transferTaxRate for burn tax, 5000 -> 50%
    uint256 public liquidityRate = 0;           // percent of transferTaxRate for liquidity tax, 5000 -> 50%
    ...
    // StarSwap Fee hander
    address public starSwapFeeDistributor;
    ...
    event StarSwapFeeDistributorUpdated(address indexed operator, address indexed starSwapFeeDistributor);
     ...
     */
    constructor() ERC20("StarSwap Token", "STAR") {
        _operator = _msgSender();
        emit OperatorTransferred(address(0), _operator);

        starSwapRouter = IUniswapV2Router02(0xF4e6251852144B9b3e268a175FcE8C0C47e6419E);
    ...
    function transferOperator(address newOperator) public onlyOperator {
        ...
        _excludedFromAntiWhale[_operator] = false;
        excludeFromFees(_operator, false);
    ...
    function setBurnAndLiquidityRate(uint256 _burnRate, uint256 _liquidityRate) external onlyOperator{
        uint256 _totalFeeRate = _burnRate.add(_liquidityRate);
        require(_totalFeeRate == 10000, "StarSwap : Total Fee Rate is wrong");  // total Fee Rate must be 100%.
    ...
    function blacklistAddress(address account, bool value) external onlyOperator {
        _isBlacklisted[account] = value;
    }

    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner.
    function mint(address _to, uint256 _amount) external onlyOwner {
        require(totalSupply() + _amount <= MAX_TOTAL_SUPPLY, "Too many token mint!");
    ...
    function _transfer(address sender, address recipient, uint256 amount) 
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            if (
                ...
                uint256 totalFeeRate = burnRate.add(liquidityRate);
                ...
                uint256 liquidityAmount =  contractTokenBalance.sub(burnAmount);
        ...
        if (!swapping && !locked) {
            ...
            if (
                sender != starSwapPair 
                && recipient != starSwapPair
                && recipient != address(starSwapRouter)
                && sender != address(starSwapRouter) 
            ...
            // check the current liquidity in the StarSwap Fee Distributor
            if (
                sender != starSwapPair 
                && recipient != starSwapPair
                && recipient != address(starSwapRouter)
                && sender != address(starSwapRouter) 
                && starSwapFeeDistributor != address(0) 
                && StarSwapFeeHandler(starSwapFeeDistributor).getOperator() == address(this)) 
            {
                StarSwapFeeHandler(starSwapFeeDistributor).checkFeeLiquidityBalance();
            }
    ...
    function updateLottery(address _toLottery) external onlyOperator {
        ...
        require(totalSupply() + lotteryPrizeReserve <= MAX_TOTAL_SUPPLY, "Too many token mint!");
     ...
     * Can only be called by the current operator.
     */
    function updateStarSwapFeeDistributor(address _feeDistributor) public onlyOperator {
        require(_feeDistributor != address(0), "Update Fee Distributor: Wrong address.");
      
        starSwapFeeDistributor = _feeDistributor;
        
        emit StarSwapFeeDistributorUpdated(msg.sender, starSwapFeeDistributor);
    }
